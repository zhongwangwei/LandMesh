module MOD_mask_postproc
    ...
    USE MOD_file_preprocess, only : Contain_Read, Unstructured_Mesh_Save, Unstructured_Mesh_Read, earthmesh_info_save! Add by Rui Zhang
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   use for earthmesh !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    SUBROUTINE mask_postproc_Earth()
        ...
        print*, lndname, 'reading in the SUBROUTINE mask_postproc_Earth'
        ...
        print*, "sum_land_ustr = ", sum_land_ustr
        print*, "sum_sea_ustr = ", sum_sea_ustr
        print*, "patchtypes_make finish"
        print*, ""
        ...
        print*, "start to read unstructure mesh data in the module MOD_mask_make"
        ! 读取未细化初始网格数据
        ...
        print*, "The unstructured grid data reading have done in the SUBROUTINE mask_postproc_Earth"
        print*, ""
        ...
        if (mode_grid == 'tri') then
            print*, "去除不存在三角形网格前", ustr_points, "个三角形网格"
            print*, "去除不存在三角形网格后", ustr_points_new, "个三角形网格"
            print*, "去除不存在三角形网格前", ustr_bounds, "个六边形网格"
            print*, "去除不存在三角形网格后", ustr_bounds_new, "个六边形网格(含虚假六边形)"
            print*, ""
        else if (mode_grid == 'hex') then
            print*, "去除不存在六边形网格前", ustr_points, "六边形网格"
            print*, "去除不存在六边形网格后", ustr_points_new, "六边形网格"
            print*, "去除不存在六边形网格前", ustr_bounds, "三角形网格"
            print*, "去除不存在六边形网格后", ustr_bounds_new, "三角形网格(含虚假三角形)"
            print*, ""
        end if
        ...
        print*, "n_ustr_ngr_vertex_f range "
        print*, minval(n_ustr_ngr_vertex_f(2:ustr_bounds_f)), maxval(n_ustr_ngr_vertex_f(2:ustr_bounds_f)) ! 单独存放   

        ! 删除部分信息后，会造成数据编号的不连续呢，
        ! ustr_center_f和ustr_vertex_f的顺序都是对的，ustr_ngr_vertex_f的顺序也是对的，但ustr_ngr_center_f却不是
        print*, "sort order of ustr_ngr_center_f (step1)"
        !  步骤1：提取唯一顶点编号
        ...
        print*, "sort order of ustr_ngr_center_f (step2)"
        ! 步骤2：排序并重新编号顶点
        ...
        print*, "sort order of ustr_ngr_center_f (step3)"
        ! 步骤3：根据新的顶点编号重新编号中心点
        ...
        print*, "minval(ustr_ngr_center_f) = ", minval(ustr_ngr_center_f(1:3, 2:ustr_points_f))
        print*, "maxval(ustr_ngr_center_f) = ", maxval(ustr_ngr_center_f(1:3, 2:ustr_points_f))
        print*, "sort order of ustr_ngr_center_f finish"
        ...
        print*, "earthmesh_info save finish"
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   use for landmesh !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    SUBROUTINE mask_postproc_Lnd()
        ...
        write(nxpc, '(I4.4)') NXP
        lndname = trim(file_dir) // 'contain/contain_'// trim(mesh_type) //'_domain_NXP'//trim(nxpc)//'_'//trim(mode_grid)//'.nc4'
        print*, lndname, 'reading in the SUBROUTINE mask_postproc_Lnd'
        CALL Contain_Read(lndname, ustr_points, numpatch, ustr_id, ustr_ii, IsInDmArea_ustr_read)
        ...
        ! 读入的数据除了第一个图形，其他图形都是存在的
        call Unstructured_Mesh_Read(lndname, sjx_points, lbx_points, mp, wp, ngrmw, ngrwm, n_ngrwm)
        print*, "The unstructured grid data reading have done in the SUBROUTINE mask_postproc_Lnd"
        print*, ""
        print*, "In total, triangular mesh number: ", sjx_points, "polygon mesh number: ", lbx_points
        print*, ""
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   use for oceanmesh !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    SUBROUTINE mask_postproc_Ocn()
        ...
        ! 读入的数据除了第一个图形，其他图形都是存在的
        call Unstructured_Mesh_Read(lndname, sjx_points, lbx_points, mp, wp, ngrmw, ngrwm, n_ngrwm)
        print*, "The unstructured grid data reading have done in the SUBROUTINE mask_postproc_Ocn"
        print*, ""
        print*, "In total, triangular mesh number: ", sjx_points, "polygon mesh number: ", lbx_points
        print*, ""
        ...
        ! 但是如果边界上含有的元素很少怎么办
        lndname = trim(file_dir) // 'contain/contain_'// trim(mesh_type) //'_domain_NXP'//trim(nxpc)//'_'//trim(mode_grid)//'.nc4'
        print*, lndname, 'reading in the SUBROUTINE mask_postproc_Ocn'
        CALL Contain_Read(lndname, ustr_points, numpatch, ustr_id, ustr_ii, IsInDmArea_ustr_read)
        IsInDmArea_ustr = IsInDmArea_ustr_read
        ...
        ! 由新center中心点指向顶点，再统计顶点个数
        if (mode_grid == 'tri') then
            print*, "去除不存在三角形/陆地三角形网格前", ustr_points, "个三角形网格"
            print*, "去除不存在三角形/陆地三角形网格后", ustr_points_new, "个海洋三角形网格"
            print*, "去除不存在三角形/陆地三角形网格前", ustr_bounds, "个六边形网格"
            print*, "去除不存在三角形/陆地三角形网格后", ustr_bounds_new, "个六边形网格(含虚假六边形)"
            print*, ""
        else if (mode_grid == 'hex') then
            print*, "去除不存在六边形/陆地六边形网格前", ustr_points, "六边形网格"
            print*, "去除不存在六边形/陆地六边形网格后", ustr_points_new, "六边形网格"
            print*, "去除不存在六边形/陆地六边形网格前", ustr_bounds, "三角形网格"
            print*, "去除不存在六边形/陆地六边形网格后", ustr_bounds_new, "三角形网格(含虚假三角形)"
            print*, ""
        end if

        if (mode_grid == 'tri') then
            print*, "IsInDmArea_ustr Renew start" ! only use for tri
            ...
            CALL Data_Renew(ustr_points, ustr_bounds, ustr_ngr_center, n_ustr_ngr_center, ustr_points_new, ustr_bounds_new, &
                            ustr_ngr_center_new, ustr_ngr_vertex_new, n_ustr_ngr_center_new, n_ustr_ngr_vertex_new)
            print*, "海陆关系调整后(step1)，", ustr_points_new, "个三角形网格"
            print*, "海陆关系调整后(step1)，", ustr_bounds_new, "个六边形网格(含虚假六边形)"

            ! for connect!'
            hhh = 1
            k = 1
            do while (hhh /= ustr_points_new)
                print*, "k= ", k, "hhh = ", hhh, "ustr_points_new = ", ustr_points_new
                ...
                CALL Data_Renew(ustr_points, ustr_bounds, ustr_ngr_center, n_ustr_ngr_center, ustr_points_new, ustr_bounds_new, &
                                ustr_ngr_center_new, ustr_ngr_vertex_new, n_ustr_ngr_center_new, n_ustr_ngr_vertex_new)
                print*, "海陆关系调整后(step2)，", ustr_points_new, "个三角形网格"
                print*, "海陆关系调整后(step2)，", ustr_bounds_new, "个六边形网格(含虚假六边形)"
                print*, "IsInDmArea_ustr Renew finish"
                print*, ""
                hhh = ustr_points_new
                print*, "narrow_waterway_widen start"
                ...
                CALL Data_Renew(ustr_points, ustr_bounds, ustr_ngr_center, n_ustr_ngr_center, ustr_points_new, ustr_bounds_new, &
                                ustr_ngr_center_new, ustr_ngr_vertex_new, n_ustr_ngr_center_new, n_ustr_ngr_vertex_new)
                print*, "narrow_waterway_widen后，", ustr_points_new, "个三角形网格"
                print*, "narrow_waterway_widen后，", ustr_bounds_new, "个六边形网格(含虚假六边形)"
                print*, "narrow_waterway_widen finish"
                print*, ""
                k = k + 1
            end do

            print*, "Isolated oceanmesh Renew start"
            ...
            print*, "Isolated oceanmesh Renew finish"
            ! 重新更新数据
            deallocate(ustr_ngr_center_new, n_ustr_ngr_center_new)
            deallocate(ustr_ngr_vertex_new, n_ustr_ngr_vertex_new) 
            CALL Data_Renew(ustr_points, ustr_bounds, ustr_ngr_center, n_ustr_ngr_center, ustr_points_new, ustr_bounds_new, &
                            ustr_ngr_center_new, ustr_ngr_vertex_new, n_ustr_ngr_center_new, n_ustr_ngr_vertex_new)
            print*, "孤立海洋网格处理后，", ustr_points_new, "个三角形网格"
            print*, "孤立海洋网格处理后，", ustr_bounds_new, "个六边形网格(含虚假六边形)"
            print*, ""
        else if (mode_grid == 'hex') then
            ! 六边形的海洋网格还需要进一步的处理
            print*, "hex do not have Special treatment now!" 
        end if
    ...
    SUBROUTINE Isolated_Ocean_Renew(ustr_bounds, ustr_points_new, ustr_bounds_new, ustr_ngr_center, n_ustr_ngr_center, n_ustr_ngr_vertex, &
        ...
        do i = 1, num_closed_curve, 1
            ...
            if (num_diff < 0) then ! 判断为孤立海洋网格
                print*, "i = ", i, "为孤立海洋网格需要去除"
                print*, "Isolated oceanmesh remove start"
                print*, "n_close_curve(i) = ", n_close_curve(i)
                ! 获取起始最外围边界信息
                num_add = 1
                do while (num_add /= 0)
                    num_add = 0
                ...
                print*, "Isolated oceanmesh remove finish"
                print*, ""
            end if
        end do
        print*, "sum(n_close_curve) = ", sum(n_close_curve) 
    ...
    SUBROUTINE narrow_waterway_widen(ustr_bounds, ustr_points_new, ustr_ngr_vertex, ustr_ngr_center_new, n_ustr_ngr_vertex, n_ustr_ngr_center_new, n_ustr_ngr_vertex_new)
        ...
        print*, "Before handle Narrow waterway bdy_num_in = ", bdy_num_in
        print*, "Handle Narrow waterway start"
        ...
        print*, "Handle Narrow waterway finish"
        print*, "After handle Narrow waterway bdy_num_in = ", bdy_num_in
        print*, ""
    ...
    SUBROUTINE bdy_connection(ustr_bounds, ustr_points_new, ustr_ngr_center_new, n_ustr_ngr_vertex, n_ustr_ngr_center_new, n_ustr_ngr_vertex_new, &
        ...
        do i = 2, ustr_bounds, 1
            ...
            print*, "i = ", i, "n_ngrvv_new(i) = ", n_ngrvv_new(i)
            print*, "ngrvv_new(1:4, i) = ", ngrvv_new(1:4, i)
    ...
    SUBROUTINE bdy_connection_closed_curve(bdy_num_in, bdy_order, bdy_ngr, num_closed_curve, num_bdy_long, close_curve, n_close_curve)
        ...
        do while(sum(bdy_alternate) > 1)
            ...
            if (present(n_close_curve)) then
                ...
                print*, "num_closed_curve = ", num_closed_curve, "闭合曲线长度为 ：", num_points
            end if
        ...
        print*, "num_bdy_long = ", num_bdy_long, "start from one"
    ...
    SUBROUTINE Data_Renew(ustr_points, ustr_bounds, ustr_ngr_center, n_ustr_ngr_center, ustr_points_next, ustr_bounds_next, &
        ...
        print*, "重新计算并储存ustr_ngr_center and ustr_center_next"
        ...
        do i = 3, bdy_num-1, 1
            if (obc_order(i) /= 1) then
                if ((obc_order(i-1) == 1) .and. (obc_order(i+1) == 1)) then
                    ...
                    print*, "j = ", j
                    do k = 1, n_ustr_ngr_vertex(j), 1
                        print*, "ustr_ngr_vertex(k, j) = ", ustr_ngr_vertex(k, j) 
                        print*, "IsInDmArea_ustr(ustr_ngr_vertex(k, j)) = ", IsInDmArea_ustr(ustr_ngr_vertex(k, j))
                    end do
                    print*, ""
                    ! turn obc to ibc
