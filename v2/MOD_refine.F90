module MOD_refine
    ...
    use MOD_grid_preprocess, only : SpringAjustment_refine, TriMeshQuality, PolyMeshQuality, GetTriangleLength, GetAngle 
    implicit none
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!! add by Rui Zhang !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    SUBROUTINE refine_loop(exit_loop)
        ...
        print*, "start to read unstructure mesh data in the Module MOD_refine in Line 55"
        ! 读取未细化初始网格数据
        write(nxpc, '(I4.4)') NXP
        ...
        if (step > 1 .or. any(n_ngrwm(2:lbx_points)<5)) then
            print*, "根据相邻距离筛选七边形附近点 或者 虚假多多边形"
            set_dis = halo(step)
            ...
            print*, "去除孤立细化三角形前，需要细化的三角形:", num_ref
            do i = num_vertex + 1, sjx_points, 1
            ...
            print*, "去除孤立细化三角形后，需要细化的三角形：", num_ref
            if (num_ref == 0) then
                exit_loop = .true.   
                return
            end if
        ...
        print*, "细化的三角形+外围halo三角形个数：", num_halo
        !----------------------------------------------------------------
        ...
        if (Istransition) then
            ...
            print*, "bdy_connection_refine finish"
            print*, ""
            ...
            print*, "分段个数num_bdy_refine_segment为：", num_bdy_refine_segment
            print*, "bdy_refine_segment_make finish"
            print*, ""
            ...
            if (.not. weak_concav_eliminate) then ! 消除弱凹就肯定是拓展为六边形
                if (num_ref == 0) then
                    ...
                    print*, "num_ref_weak_concav = ", num_ref_weak_concav
                    print*, "num_weak_concav_segment = ", num_weak_concav_segment
                    print*, "num_weak_concav_pair = ", num_weak_concav_pair
                    print*, "weak_concav_segment_make finish"
                    print*, ""
                end if
            end if
            ...
            do while(TransitionRow_iter <= set_dis_in)
                print*, "TransitionRow_iter = ", TransitionRow_iter, "in Line 364"
                ...
                if (num_ref == 0) then
                    ...
                    print*, "开始细化相邻三角形经过初步细化数为1的三角形 in Line 402"
                    print*, "iter =", iter, "num =", num_ref
                    iter = iter + 1
                ...
                if (TransitionRow_iter == 3) then
                    ...
                    print*,"num_weak_concav_pair turn to zero"
                end if
                ...
                if (num_weak_concav_pair /= 0) then
                    print*, "weak_concav_special start"
                    ...
                    print*, "weak_concav_special finish"
                    print*, ""
                end if

                num_ref = INT(sum(ref_sjx))
                if (num_ref == 0) then
                    print*,"NO 相邻三角形之间的反向一分二细化"
                else
                    print*, "开始相邻细化三角形之间的反向一分二细化"
                    print*, "iter =", iter, "num =", num_ref
                    iter = iter + 1
                ...
                print*, "相邻细化三角形之间的反向一分二细化完成 in Line 458"
                print*, ""
                ...
                print*, "max value of num_end = ", num_end, "ref_sjx_segment_temp"
                print*, ""
                ...
                if (num_weak_concav_pair + num_weak_concav_segment == 0) then
                    print*, " weak_concav_eliminate turn to TRUE"
    ...
    SUBROUTINE GetTriangleDis(num_vertex, set_dis, num_ref, sjx_points, ngrmw, ngrwm, n_ngrwm, ref_select, mp_dis)
        ...
        print*, "shape(mp_dis) = ", shape(mp_dis)
    ...
    SUBROUTINE iterC_judge(set_dis_in, sjx_points, lbx_points, ngrmm, ngrmw, ngrwm, n_ngrwm, mrl_new, ngr_mrl_new, ref_lbx)
        ...
        !     end do
        !     ! if (num == 3) print*, "同一个三角形的三个顶点都是七边形"
    ...
    SUBROUTINE bdy_connection_refine(sjx_points, lbx_points, ngrmw, mrl_new, ngrwm, n_ngrwm, wp, num_closed_curve, close_curve, n_close_curve)
        ...
        print*, "bdy_num_in_save = ", bdy_num_in_save
        ...
        do i = num_center + 1, lbx_points, 1
            ...
            print*, "i = ", i, "n_ngrvv_refine(i) = ", n_ngrvv_refine(i)
            print*, "ngrvv_refine(1:4, i) = ", ngrvv_refine(1:4, i)
            print*, ""
            ! print*, "n_ngrvv_refine(i) must be 0 or 2! adjust scheme writing now!"
        ...
        print*, "ngrvv_refine_adjust finish"
        ...
        print*, "get close_curve and n_close_curve start"
        ...
        print*, "get close_curve and n_close_curve finish"
    ...
    SUBROUTINE bdy_connection_refine_closed_curve(bdy_num_in, bdy_order, bdy_ngr, num_closed_curve, num_bdy_long, close_curve, n_close_curve)
        ...
        print*, "bdy_num_in = ", bdy_num_in, "in Line 1288 SUBROUTINE bdy_connection_refine_closed_curve"
        ...
        do while(sum(bdy_alternate) > 1)
            ...
            ! print*, "end at : ", bdy_end
            ! print*, "ngr_select = ", ngr_select
            ...
            if (present(n_close_curve)) then
                ...
                num_points = num_points + 1
                print*, "num_closed_curve = ", num_closed_curve, "闭合曲线(尾部空1)点位个数为 ：", num_points
            end if
        ...
        if (.not. present(n_close_curve)) then
            ...
            print*, "num_bdy_long = ", num_bdy_long, "start from two"
        end if
        ...
        do i = 1, num_closed_curve, 1
            if (set_dis_in == 1) then
                ...
                if (j /= 1) then
                    ...
                    print*, "j = ", j, "need to modify order of close_curve(1:n_close_curve(i), i)"
                ...
                do j = 1, n_close_curve(i)-1, 1
                    ...
                    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    print*, "num = ", num, "num_segement = ", num_segement
                    ...
                    if ((segement_start_end(j+k, 2) - segement_start_end(j+k, 1)) < INT((set_dis_in+1)/2)) then ! 长度4，最大区间3，分为2+2
                        print*, "refine sjx in the segement :", segement_start_end(j+k, 2) - segement_start_end(j+k, 1)
                        print*, "j+k = ", j+k
                        print*, "segement_start_end(j+k, 2) = ", segement_start_end(j+k, 2)
                        print*, "segement_start_end(j+k, 1) = ", segement_start_end(j+k, 1)
                        print*, "不满足最小区间要求",INT((set_dis_in+1)/2),"，数组修改"
                        print*, ""
    ...
    SUBROUTINE sharp_concav_lop_judge(set_dis_in, num_ref, num_bdy_refine_segment, mrl_new, ngrmm, ngrmw_new, sjx_child, bdy_refine_segment, bdy_refine_segment_old, n_bdy_refine_segment, ref_sjx_segment_temp, n_ref_sjx_segment_temp)
        ...
        do i = 1, num_bdy_refine_segment, 1
            ...
            ! print*, "n_ref_sjx_segment_temp(i) = ", n_ref_sjx_segment_temp(i)
            do k = 1, n_ref_sjx_segment_temp(i), 4
                ! 在相邻位置获取另一端的数据
                ! print*, "k = ", k
                ref_sjx_segment_temp(k+2:k+3, i) =  ref_sjx_segment_temp(num_end-k:num_end-k+1, i)
            end do
            ! print*, "after ref_sjx_segment_temp(1:num_end, i) = ", ref_sjx_segment_temp(1:num_end, i)
            ! print*,""
        end do
    ...
    SUBROUTINE weak_concav_lop_judge(set_dis_in, num_ref, num_bdy_refine_segment, num_ref_weak_concav, num_weak_concav_segment, num_weak_concav_pair, mrl_new, ngrmm, ngrmw_new, sjx_child, &
        ...
        if (num_weak_concav_segment /= 0) then
            do i = 1, num_end, 1
                ...
                ! if (n_weak_concav_segment(i) == 0) cycle
                print*, "n_weak_concav_segment(i) = ", n_weak_concav_segment(i), "说明存在两侧长度大于1的弱凹 in Line 1917"
    ...
    SUBROUTINE Delaunay_Lop(iter, num_ref, num_mp, num_wp, mp_new, wp_new, ngrmw_new, ref_sjx_segment)            
        ...
        do k = 1, num_ref/2, 1
            ...
            if (i==0 .or. j==0) then
                print*, "i = ", i, "j = ", j, "in Line 1971 SUBROUTINE Delaunay_Lop"
