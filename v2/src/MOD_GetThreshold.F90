MODULE MOD_GetThreshold
    ...
    SUBROUTINE GetThreshold(exit_loop)
        ...
        write(io6, *)   "开始读取非结构网格数据 in the GetThreshold.F90"
        ...
        write(io6, *)   "非结构网格数据读取完成 in the GetThreshold.F90"
        write(io6, *)   ""
        ...
        if (any(refine_onelayer .eqv. .true.) .or. &
            ...
            write(io6, *)   "threshold_Calculation for refine_onelayer or refine_twolayer"
            ...
            write(io6, *)   "threshold_Calculation for refine_onelayer or refine_twolayer : done"
        end if
        write(io6, *)   "threshold_Calculation finish"
    ...
    SUBROUTINE mean_std_cal2d(ii, sjx_points, flag_temps, mp_id, mp_ii, var2d, num_onelayer, p_num)
        ...
        if (refine_onelayer(2*ii-1)) then
            write(io6, *)   trim(RL_onelayer(ii))//"_m", minval(tempm, mask=(tempm /= 0.)), maxval(tempm, mask=(tempm /= 0.)) 
            num_onelayer = num_onelayer + 1
        ...
        write(io6, *)   trim(RL_onelayer(ii))//"_s", minval(temps, mask=(temps /= 0.)), maxval(temps, mask=(temps /= 0.))
        num_onelayer = num_onelayer + 1
        allocate(output2d(num_onelayer)%varms(sjx_points))
    ...
    SUBROUTINE mean_std_cal3d(ii, sjx_points, flag_temps, mp_id, mp_ii, var3d, num_twolayer, p_num)
        ...
        if (refine_twolayer(2*ii-1)) then 
            ...
            write(io6, *)   trim(RL_twolayer(ii))//"_m", minval(tempm, mask=(tempm /= 0.)), maxval(tempm, mask=(tempm /= 0.))
            do i = 2, sjx_points, 1
